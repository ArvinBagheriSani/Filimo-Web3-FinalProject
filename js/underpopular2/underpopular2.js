const underpopular2 = async () => {
  try {
    let data = await fetch("http://localhost:3000/underpopular2");
    let res = await data.json();
    console.log(res);
    let popular2 = res.map((elem) => {
    ...
    document
      .querySelector(".underpopular2")
      .insertAdjacentHTML("beforeend", popular2);
  } catch (error) {
    console.log(error.message);
  }
};
export default underpopular2;
